...
while True:
    if '666' in str(bad):                       #문자열 '666' 으로 만든 후 비교
        count += 1                              #count는 횟수 증가 n까지 비교해야되서  666있는거 한번 찾을때마다 +1로 늘어남
        if count == n:                          #count ==n 이면 n번째 횟수 값을 찾았다는 말
            print(bad)                          # n번째의 bad값 출력
            break                               #조건탈출
    bad += 1                                #1씩더해주며 검사..
