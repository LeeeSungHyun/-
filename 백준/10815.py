...
n = int(input())                                        # 카드의 개수
card = set(map(int,input().split()))                    # set함수 (중복제거)     숫자카드에 적혀있는 수
m = int(input())                                        # 비교할 카드 개수
find_list = list(map(int,input().split()))              # 비교할 카드에 적혀있는 정수


for i in range(m):                                      # 비교할 카드 개수만큼 비교(반복)
    if find_list[i] not in card:                        # 리스트의 0번째자리 ~ m-1 자리까지 
        print(0,end=' ')        
    else:
        print(1,end=' ')
