...
# 문제를 단순화하기 위해서 호텔은 직사각형 모양이라고 가정하자. 각 층에 W 개의 방이 있는 H 층 건물이라고 가정하자 (1 ≤ H, W ≤ 99). 그리고 엘리베이터는 가장 왼쪽에 있다고 
# 가정하자(그림 1 참고). 이런 형태의 호텔을 H × W 형태 호텔이라고 부른다. 호텔 정문은 일층 엘리베이터 바로 앞에 있는데, 정문에서 엘리베이터까지의 거리는 무시한다. 
...
for i in range(T):
    ...
    floor = n%h                             # 몇번째손님 % 층 = 나머지        #층
    long = (n//h) + 1                       # 몇번째손님 // 층 +1 = 거리    # 거리(방호수)

    if floor == 0:                          # 꼭대기 층인 경우?..
        floor = h
        long = n//h

    print(floor * 100 + long)
