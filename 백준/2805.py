...
start = 0                       # 가장 긴 나무의 처음시작 값
end = max(array) - 1            # 가장 긴 나무의 길이 (최종값)                   

while start <= end:
    mid = (start + end) // 2    # 중간지점 값   (= 9)
    wood = 0                    #벌목된 나무 총길이
    for i in array:             # [20, 15, 10, 17]순서로 i에 들어감
        if i >= mid:                
            wood += i - mid     # 절단기로 자른 후 나무길이의 총합 (11+6+1+8) = 26
    if wood >= m:               # 26 >= 7        나무잘라서 갖을 수 있는 길이 >=  최소필요값   true면 right로..
        start = mid + 1         # 시작점 10으로 설정
    else:
        end = mid - 1                 # left로..
